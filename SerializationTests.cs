...
namespace Test_Assignment_3
{
	public class SerializationTests
	{
		...
		public void TestDeSerialization()
		{
			...
			{
				Assert.That(deserializedUsers[i].Id, Is.EqualTo(users[i].Id));
				Assert.That(deserializedUsers[i].Name, Is.EqualTo(users[i].Name));
				Assert.That(deserializedUsers[i].Email, Is.EqualTo(users[i].Email));
				Assert.That(deserializedUsers[i].Password, Is.EqualTo(users[i].Password));
			}
		}
